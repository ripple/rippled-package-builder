...
export function DeployRPMToStaging(rpm) {
  ...
  const RPM = `-e "COMMIT_HASH=${rpm.commit_hash}" -e "MD5SUM=${rpm.md5sum}" -e "RIPPLED_VERSION=${rpm.rippled_version}" -e "YUM_REPO=${rpm.yum_repo}"`

  spawnDocker(`${DOCKER_RUN} -e "S3_BUCKET=${rpm.s3_bucket}" -e "S3_KEY=${rpm.s3_key}" -e "AWS_REGION=${rpm.aws_region}" ${RPM} ${image}`)
}
...
export function TestStagingRPM(rpm) {
  ...
  spawnDocker(`${DOCKER_RUN} ${RPM} ${image}`)
}

...
export function RPMFromCommit(commitHash, s3Bucket, yumRepo) {
  ...
  spawnDocker(`${DOCKER_RUN} -e "COMMIT_HASH=${commitHash}" -e "YUM_REPO=${yumRepo}" -e "GPG_PASSPHRASE=${process.env.GPG_PASSPHRASE}" -e "S3_BUCKET=${s3Bucket}" -e "SQS_QUEUE_UPLOADED=${process.env.SQS_QUEUE_UPLOADED}" ${image}`)
}
...
export function DeployRPMToStaging(rpm) {
  ...
  spawnDocker(`${DOCKER_RUN} -e "S3_BUCKET=${rpm.s3_bucket}" -e "S3_KEY=${rpm.s3_key}" ${RPM} -e "SQS_QUEUE_DEPLOYED=${process.env.SQS_QUEUE_DEPLOYED}" ${image}`)
}

...
export function BuildRPMFromCommit(commitHash, s3Bucket, yumRepo) {
  const image = 'rippled-rpm-builder'

  spawnDocker(`${DOCKER_RUN} -v /etc/gpg:/opt/rippled-rpm/gpg -e "COMMIT_HASH=${commitHash}" -e "YUM_REPO=${yumRepo}" -e 'GPG_PASSPHRASE=${process.env.GPG_PASSPHRASE}' -e "S3_REGION=${process.env.S3_REGION}" -e "S3_BUCKET=${s3Bucket}" -e "SQS_QUEUE_UPLOADED=${process.env.SQS_QUEUE_UPLOADED}" ${image}`)
}
...
export function DeployRPMToStaging(rpm) {
  ...
  const ANSIBLE = `-e "STAGING_YUM_HOST=${process.env.STAGING_YUM_HOST}" -e "STAGING_YUM_USER=${process.env.STAGING_YUM_USER}" -e "STAGING_YUM_PEM_FILE=${process.env.STAGING_YUM_PEM_FILE}"`

  spawnDocker(`${DOCKER_RUN} -e "S3_REGION=${process.env.S3_REGION}" -e "S3_BUCKET=${rpm.s3_bucket}" -e "S3_KEY=${rpm.s3_key}" ${RPM} ${ANSIBLE} -e "SQS_QUEUE_DEPLOYED=${process.env.SQS_QUEUE_DEPLOYED}" ${image}`)
}
